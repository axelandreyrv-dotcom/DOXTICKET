...
class AdminDashboardTest extends TestCase
{
    ...
    public function test_superadmin_sees_configured_donation_links_without_unconfigured_providers(): void
    {
        Config::set('doxticket.donations.paypal_url', 'https://paypal.me/doxticket');
        Config::set('doxticket.donations.github_sponsors_url', 'https://github.com/sponsors/doxticket');
        ...
        $this->actingAs($superadmin)
            ->get('/admin')
            ->assertOk()
            ->assertSee('Apoyar DoxTicket')
            ->assertSee('PayPal')
            ->assertSee('GitHub Sponsors')
            ->assertDontSee('Buy Me a Coffee')
            ->assertSee('href="https://paypal.me/doxticket"', false)
            ->assertSee('rel="noreferrer"', false);
    }

    public function test_admin_dashboard_hides_unsafe_donation_urls(): void
    {
        Config::set('doxticket.donations.paypal_url', 'javascript:alert(1)');
        ...
        $this->actingAs($superadmin)
            ->get('/admin')
            ->assertOk()
            ->assertDontSee('Apoyar DoxTicket')
